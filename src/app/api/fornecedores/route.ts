...
export async function POST(req: NextRequest) {
  ...
  try {
    const { nome, cpfCnpj, email, telefone, capitalTotal, senhaPortal } = await req.json();
    ...
    const usuario = await prisma.usuario.create({
      data: { nome, email, senha: senhaHash, perfil: 'FORNECEDOR' },
    });

    const fornecedor = await prisma.fornecedor.create({
      data: {
        usuarioId: usuario.id, nome, cpfCnpj, email, telefone,
        capitalTotal: capitalTotal ? parseFloat(capitalTotal) : 0,
      },
    });
